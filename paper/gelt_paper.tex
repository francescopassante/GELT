...
near-unit overlap --- the trained attention field is unbiased ($+1.2\%$) while
the classical smeared basis and the untrained network read $11\%$ and $10\%$
low. The structural statement holds already at random initialization; what
...
exponents, $0.62$ classical and $0.66$ attention, agree with the $0.626$
...
\midrule
$\xi$ predicted & (fixes $\xi_0$) & $3.048$ & $4.331$ & $6.410$\\
$\xi$ measured & $2.137(18)$ & $3.051(22)$ & $4.279(33)$ & $6.355(108)$\\
deviation & --- & $\bm{+0.1\%}$ & $\bm{-1.2\%}$ & $\bm{-0.9\%}$\\
\bottomrule
\end{tabular}
\end{center}

\paragraph{Sampling.} One caution is worth stating because it cost us a run. In
the broken phase the slowest mode is \emph{not} the critical one: tunnelling
...
$\tau_{\rm int}(M)=1575$. Thermalization and decorrelation must therefore be
fixed from $\tau_{\rm int}$ of the magnetization, not of the energy and not from
$\xi^z$; we require $n_{\rm therm}\ge20\,\tau_{\rm int}(M)$ and
$n_{\rm skip}\ge2\,\tau_{\rm int}(M)$ and discard any cell that fails. It also
makes the \emph{larger} lattice the better-behaved one, inverting the usual
intuition.

\subsection{Which operator is right}

Table~\ref{tab:dual} is the comparison. The trained attention field is unbiased
against the exact answer; the classical smeared basis and the untrained network
are low by $11\%$ and $10\%$, and the ordering is exactly the one their $A_0$
predicts.

\begin{table}[t]
\caption{Fractional deviation of each gauge-side operator from the dual ground
truth, with the significance of the deviation in parentheses. $\beta=0.7600$ is
excluded: its true $\xi\approx10$ exceeds what $L=24$ can hold, so all three
operators are $50$--$58\%$ low together and the point discriminates nothing.}
\label{tab:dual}
\begin{ruledtabular}
\begin{tabular}{lcccc}
$\beta$ & $\xi$ exact & classical & attention & attention\\
        &             &           & trained   & random\\
\colrule
$0.7450$ & $2.137(18)$ & $-4.6\%\,(0.7\sigma)$ & $+5.0\%\,(0.8\sigma)$
         & $-2.9\%\,(0.5\sigma)$\\
$0.7520$ & $3.051(22)$ & $-23.2\%\,(4.3\sigma)$ & $-9.4\%\,(2.1\sigma)$
         & $-19.4\%\,(3.8\sigma)$\\
$0.7560$ & $4.279(33)$ & $-4.7\%\,(0.7\sigma)$ & $+4.8\%\,(0.7\sigma)$
         & $-4.7\%\,(0.8\sigma)$\\
$0.7585$ & $6.355(108)$ & $-12.0\%\,(2.2\sigma)$ & $+4.4\%\,(0.8\sigma)$
         & $-13.9\%\,(2.7\sigma)$\\
\colrule
\emph{mean} & & $\bm{-11.1\%}$ & $\bm{+1.2\%}$ & $\bm{-10.2\%}$\\
\end{tabular}
\end{ruledtabular}
\end{table}
...
$\beta$ & $0.7450$ & $0.7520$ & $0.7560$ & $0.7585$\\
\midrule
$A_0(\sigma)$ & $0.94$ & $0.95$ & $0.93$ & $0.98$\\
$A_0(\varepsilon_t)$ & $0.60$ & $0.50$ & $0.41$ & $0.35$\\
...
$0.622$ and $0.656$, against $0.626$ from the dual fitted identically on the same
...
compared --- and the trained attention field turns out to be unbiased against it
while the classical smeared basis and the untrained network are $11\%$ and
$10\%$ low. That converts $\Delta A_0$ from a statement about operator quality
