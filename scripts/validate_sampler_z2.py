...
for i, L in enumerate(LS_2D):
    ...
    ax.errorbar(
        BETAS_2D, ms, yerr=es, fmt="o-", ms=3, capsize=2, color=COL[i], label=f"L = {L}"
    )
ax.set_xlabel("β")
ax.set_ylabel("⟨P⟩")
...
for i, L in enumerate(LS_3D):
    ...
    ax.errorbar(
        BETAS_3D, ms, yerr=es, fmt="o-", ms=3, capsize=2, color=COL[i], label=f"L = {L}"
    )
ax.set_xlabel("β")
ax.set_ylabel("⟨P⟩")
...
fig.tight_layout()
plt.savefig("results/sampler/sampler_validation_z2.png", dpi=150, bbox_inches="tight")
print("Saved results/sampler/sampler_validation_z2.png")
plt.show()
