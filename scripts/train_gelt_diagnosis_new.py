...
from gelt.blocks_new import GELT
from gelt.lattice import rectangular_wilson_loop

"""
========================================================================================
 Diagnostic training script for the SPLIT-BLOCK GELT (notes/new_architecture.md §4).

 Mirrors train_gelt_diagnosis.py — same loop, same dead-signal probes, same Wilson-
 loop target — but trains the new ``gelt.blocks_new.GELT`` (GEAttention + GEFFN)
 instead of the fused GEMHSA. The per-layer α diagnostic is extended to print the
 sub-block kind ('A' / 'F' / 'M') so you can see whether the routing path or the
 multiplicative path is engaging first under the chosen schedule.

 The expected qualitative result vs. the fused GEMHSA baseline (per §5): the wall
 at the 3×3 Wilson loop moves, because routing (Attn) and multiplication (FFN)
 are no longer coupled into a single gradient step.
========================================================================================
"""
...
def train_model(
    ...
    for epoch in epoch_bar:
        ...
        # only the MLP receives gradient on step 0; α and the sub-block params
        ...
        #
        # Split-block extension: each α string is prefixed by the sub-block
        # kind ('A' for GEAttention, 'F' for GEFFN, 'M' for legacy GEMHSA) so
        # you can see whether routing or multiplication is engaging first.
        # On the §5 schedule (ffn_alpha_init > attn_alpha_init) the F-prefixed
        # values should leave zero before the A-prefixed ones.
        if hasattr(model, "gemhsa_models"):
            kinds = getattr(model, "_kinds", ["?"] * len(model.gemhsa_models))
            alphas = [
                f"{k}:{layer.alpha.item():+.3f}"
                for k, layer in zip(kinds, model.gemhsa_models)
            ]
...
if __name__ == "__main__":
    ...
    # The 3×3 target is the one that walled the fused GEMHSA across every
    # hyperparameter sweep (notes/new_architecture.md §1) — the canonical
    # benchmark for the split block.
    ...
    train_parameters = {
        ...
        "epochs": 3000,
        "patience": 3000,
        # Distinct checkpoint name so the split-block run doesn't clobber
        # the fused-GEMHSA checkpoint from train_gelt_diagnosis.py.
        "checkpoint_path": "best_gelt_split.pth",
    }

    # Split-block GELT for the 3×3 per-site Wilson loop target.
    # ``n_layers=4`` with the default pattern expands to "AFAFAFAF" — four
    # attention sub-blocks interleaved with four FFN sub-blocks (8 sub-blocks
    # total). Same loop-doubling reach as the previous 4-layer fused GEMHSA
    # stack (each fused layer was also 1 routing + 1 multiplicative step in
    # one pass), but routing and multiplication now have decoupled gradients.
    # See notes/new_architecture.md §4.3.
    model_parameters = {
        "gaugegroup": gaugegroup,
        "L": L,
        "D": D,
        ...
        "nhead": 1,
        "n_layers": 4,
        # Override the default pattern here if you want unequal counts. E.g.
        # ``"pattern": "AFFAFFAFFAFF"`` for "2 attn + 4 L-Bilin per group"
        # (§8.4 — three multiplicative steps per routing step). Leaving it
        # unset = "AFAFAFAF".
        # "pattern": "AFFAFFAFFAFF",
        "d_qkv": 16,
        "gate": "softplus",
        ...
        "reduction": "none",
        # Decoupled-ReZero schedule from notes/new_architecture.md §5:
        # warm-start the FFN (L-Bilin) α larger than the attention α so the
        # multiplicative path engages first — it's the part that actually
        # creates new loop content, attention should concentrate on top of an
        # already non-trivial bilinear feature space.
        "ffn_alpha_init": 0.5,
        "attn_alpha_init": 0.1,
        # init_scale=10 lifts score magnitudes off the near-uniform softmax
        # floor so attention has per-offset signal to learn from on epoch 0.
        "init_scale": 10.0,
        "mlp_zero_init": False,
        ...
        # sub-block working width from the input dimensionality so intermediate
    ...
    # Print the resolved pattern + per-kind sub-block counts so the run log
    # records the exact split-block configuration trained.
    n_attn = model._kinds.count("A")
    n_ffn = model._kinds.count("F")
    n_legacy = model._kinds.count("M")
    print(
        f"GELT (split) | pattern={model.pattern} "
        f"({n_attn} attn + {n_ffn} ffn + {n_legacy} legacy) | "
    ...
    plt.xlabel("Epochs")
    plt.ylabel("Loss")
    plt.title("Training and Validation Loss (split-block GELT)")
    plt.legend()
    plt.grid(True)
    # Distinct plot names so this run doesn't overwrite the fused-GEMHSA
    # diagnostic outputs from train_gelt_diagnosis.py.
    plt.savefig("gelt_split_loss.png", dpi=150, bbox_inches="tight")
    plt.close()
    ...
    plt.title("True vs Predicted Values (split-block GELT, Test Set)")
    plt.grid(True)
    plt.savefig("gelt_split_scatter.png", dpi=150, bbox_inches="tight")
    plt.close()
