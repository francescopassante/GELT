"""Tests for the **trained** GELT variant, :mod:`gelt.blocks_rope`.

``tests/test_blocks.py`` covers ``blocks_bias``; this file covers the RoPE
variant that ``scripts/train_glueball.py`` actually imports, which the notes
flag as the untested one (CLAUDE.md caveat 1). Two things are pinned:

* **gauge equivariance** — the same guarantee as the bias variant,
  ``forward(Ω·W·Ω†, T(Ω·U)) == Ω · forward(W, T(U)) · Ω†``.
...
from gelt.blocks_rope import GELT, GEMHSA
...
def test_offset_gather_backward_matches_autograd(budget, monkeypatch):
    ...
    from gelt.blocks_rope import _OffsetGather
    import gelt.blocks_rope as br
